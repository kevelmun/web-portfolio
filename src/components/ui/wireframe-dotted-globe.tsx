...
export default function RotatingEarth({ width = 800, height = 600, className = "" }: RotatingEarthProps) {
  ...
  useEffect(() => {
    ...
    const readThemeColor = (token: string, fallback: string) => {
    ...
    const render = () => {
      ...
      const oceanColor = readThemeColor("--card", "#ffffff")
      const lineColor = readThemeColor("--primary", "#8839ef")
  ...
  return (
    <div className={`relative ${className}`}>
      <canvas
        ref={canvasRef}
        className="w-full h-auto rounded-2xl bg-background/50"
